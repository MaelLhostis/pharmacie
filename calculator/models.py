...
class Calculator:
    ...
    def compute(id, prix_brut, taux_remise, prix_achat_net, prix_vente_net, coef):
        ...
        if id == "prix_brut" and not taux_remise == 0:
            ...
            context["prix_achat_net"] = Calculator.prix_achat_net(
                float(taux_remise), float(prix_brut)
            )
            context["prix_vente_net"] = Calculator.prix_vente_net(
                context["prix_achat_net"], float(coef)
            )
            context["coef"] = coef
            ...
            context["taux_remise"] = Calculator.taux_remise(
                float(prix_brut), float(prix_achat_net)
            )
            context["prix_achat_net"] = prix_achat_net
            context["prix_vente_net"] = Calculator.prix_vente_net(
                float(prix_achat_net), float(coef)
            )
            context["coef"] = coef
            ...
            context["coef"] = Calculator.coef(
                float(prix_vente_net), float(prix_achat_net)
            )
